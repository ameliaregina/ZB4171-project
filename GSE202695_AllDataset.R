...
#actual code run
expr1 <- read.csv('GSE202695_counts_afterQC.csv', row.names = 1, header= TRUE)
metadata <- read.csv('GSE202695_metadata.csv', header= TRUE)
...
unique(seu.obj1.filtered@meta.data$type) # What else other than redefined_cluster
...
seu.obj1.filtered <- FindNeighbors(seu.obj1.filtered, dims = 1:50)
...
seu.obj1.filtered  <- RunUMAP(seu.obj1.filtered , dims = 1:50, n.neighbors = 100)
...
marker_test_res <- top_markers(cds1, group_cells_by="model", 
...
gene_module_df <- find_gene_modules(cds_subset[pr_deg_ids,"knn"], resolution=1e-3)
...
cds_subset <- cluster_cells(cds_subset, resolution=7.5e-3)
...
cds_3d <- reduce_dimension(cds1, max_components = 3)
