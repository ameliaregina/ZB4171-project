# script to perform trajectory analysis on GSE202695 subsets

setwd("D:/Monocle3/GSE202695") # set working directory and make sure it has all the files.
...
# function changes depending on file format
markers <- read.delim('ABC_Marker.txt', header = T) # gene metadata [if available]
...
# verify how content is organized for each data and modify accordingly
...
unique(HBRX2353.seu@meta.data$type)
...
# Check for and remove batch effects TO ADD
...
# Note to chris, check various settings for this step
# ...4.1. Learn trajectory graph ------------------------
...
# Note: Whole population analysis stops here [11/11/2022 10:17PM]

# Using Seurat Vignettes

# Using Monocle 3
# We can run this on either a subset or the whole population

# Two approaches: Regression analysis or Graph-autocorrelation analysis

# Regression Analysis --------------------------------------------
# The parameter model_formula_str should be a string specifying the model formula. 
# The model formulae you use in your tests can include any term that 
# exists as a column in the colData table, 
# including those columns that are added by Monocle in other analysis steps. 
# For example, if you use cluster_cells, you can test for genes that differ between clusters 
# and partitions by using ~cluster or ~partition (respectively) as your model formula. 
# You can also include multiple variables, for example ~embryo.time + batch, 
# which can be very helpful for subtracting unwanted effects.

subset_genes <- c("che-1",
                    "hlh-17",
                    "nhr-6",
                    "dmd-6",
                    "ceh-36",
...
# it is better to subset, else the process takes too long

# type can be changed to other terms see above
gene_fits <- fit_models(cds1, model_formula_str = "~type")
fit_coefs <- coefficient_table(gene_fits)

type_terms <- fit_coefs %>% filter(term == "type")

type_terms %>% filter (q_value < 0.05) %>%
  select(gene_short_name, term, q_value, estimate)

# produce a violin plot
plot_genes_violin(cds_subset, group_cells_by="type.bin", ncol=2) +
  theme(axis.text.x=element_text(angle=45, hjust=1))

# Note: additional steps can be found at
# https://cole-trapnell-lab.github.io/monocle3/docs/differential/
# Additionally if any of you make updates earlier than me inform the group
